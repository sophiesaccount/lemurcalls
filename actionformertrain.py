...
# 🔍 Beispiel: Vorwärtsdurchlauf
if __name__ == "__main__":
    model = WhisperActionFormer(encoder)
    dummy_hidden_states = torch.randn(2, 500, 512)  # Fake encoder output

    # Directly call heads (bypassing encoder)
    class_out = model.class_head(dummy_hidden_states)
    regr_out = model.regr_head(dummy_hidden_states)

    print("Classification Output Shape:", class_out.shape)  # (2, 500, 10)
    print("Regression Output Shape:", regr_out.shape)        # (2, 500, 2)
