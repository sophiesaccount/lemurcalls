...
class WhisperFormerDataset(Dataset):
    def __init__(self, audio_list, label_list, total_spec_columns, feature_extractor, num_classes=1, ):
    ...
    def __getitem__(self, idx):
        ...
        #print(f'input features shape: {input_features.shape}') #torch.Size([80, 3000])
        ...
        #print(f'actual_clip_duration: {actual_clip_duration}')
        start_time = clip_start / sr #0
        #print(f'start time: {start_time}')
        end_time = start_time + actual_clip_duration
        #print(f'end time: {end_time}') #30
        ...
        for onset, offset, cluster_id in zip(onset_in_clip, offset_in_clip, cluster_id_in_clip):
            ...
            offset_mask[center_frame - int(0.3*frame_len) : center_frame + int(0.3*frame_len)] = True
