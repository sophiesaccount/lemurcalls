...
def evaluate_detection_metrics_with_false_class(
    labels, predictions, overlap_tolerance=0.0001
):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute detection metrics for labels and predictions."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0001,
        help="Minimum overlap for a match (default: 0.0001)",
    )
    ...
    metrics = evaluate_detection_metrics_with_false_class(
        labels, predictions, overlap_tolerance=args.overlap_tolerance
    )
