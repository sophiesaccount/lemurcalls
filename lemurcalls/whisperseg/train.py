...
def collate_fn(batch):
    ...
    input_features = [
        torch.tensor(item["input_features"], dtype=torch.float32) for item in batch
    ]
    decoder_input_ids = [
        torch.tensor(item["decoder_input_ids"], dtype=torch.int64) for item in batch
    ]
...
def evaluate(
    ...
    num_beams=4,
    target_cluster=None,
    confusion_matrix: str = None,
    save_cm_data: str = None,
):
    ...
    for audio, label in tqdm(
        zip(audio_list, label_list),
        total=len(audio_list),
        desc="evaluate()",
        disable=is_scheduled_job(),
    ):
        prediction = segmenter.segment(
            ...
            time_per_frame_for_voting=label.get(
                "time_per_frame_for_voting", 0.001
            ),  ## for bin-wise voting, by default it is not used
        ...
        if confusion_matrix != None:
            confusion_matrix_framewise(
                prediction, label, None, 0.001, name=confusion_matrix
            )
        if save_cm_data != None:
            with open(
                f"/usr/users/bhenne/projects/whisperseg/results/{save_cm_data}.cmraw",
                "w",
            ) as fp:
                json.dump({"prediction": prediction, "label": label}, fp, indent=2)
        TP, P_pred, P_label = segmenter.segment_score(
            prediction, label, target_cluster=target_cluster, tolerance=0.02
        )[:3]
        ...
        TP, P_pred, P_label = segmenter.frame_score(
            prediction,
            label,
    ...
    precision = total_n_true_positive_segment_wise / max(
        total_n_positive_in_prediction_segment_wise, 1e-12
    )
    recall = total_n_true_positive_segment_wise / max(
        total_n_positive_in_label_segment_wise, 1e-12
    )
    ...
    precision = total_n_true_positive_frame_wise / max(
        total_n_positive_in_prediction_frame_wise, 1e-12
    )
    recall = total_n_true_positive_frame_wise / max(
        total_n_positive_in_label_frame_wise, 1e-12
    )
    f1 = 2 / (1 / max(precision, 1e-12) + 1 / max(recall, 1e-12))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="If the validation score does not improve for [patience] epochs, stop training.",
    )
    ...
    parser.add_argument(
        "--clear_cluster_codebook",
        type=int,
        help="set the pretrained model's cluster_codebook to empty dict. This is used when we train the segmenter on a complete new dataset. Set this to 0 if you just want to slighlt finetune the model with some additional data with the same cluster naming rule.",
        default=0,
    )
    parser.add_argument(
        "--num_classes",
    ...
    device = torch.device(
        "cuda:%d" % (args.gpu_list[0]) if torch.cuda.is_available() else "cpu"
    )

    model, tokenizer = load_model(
        args.initial_model_path, args.total_spec_columns, args.dropout
    )
    ...
    if args.audio_folder and args.label_folder:
        audio_path_list_train, label_path_list_train = (
            get_audio_and_label_paths_from_folders(args.audio_folder, args.label_folder)
        )
    else:
    ...
    if args.num_classes is not None:
        ...
        print(
            f"Using fixed codebook for {args.num_classes} class(es): {cluster_codebook}"
        )
    else:
        cluster_codebook = get_cluster_codebook(
            label_path_list_train, segmenter.cluster_codebook
        )
    ...
    if args.val_ratio > 0:
        (audio_list_train, label_list_train), (audio_list_val, label_list_val) = (
            train_val_split(audio_list_train, label_list_train, args.val_ratio)
        )

    audio_list_train, label_list_train = slice_audios_and_labels(
        audio_list_train, label_list_train, args.total_spec_columns
    )
    ...
    if len(training_dataset) < args.batch_size:
        print(
            f"Warning: Dataset size ({len(training_dataset)}) is smaller than batch size ({args.batch_size})"
        )
    ...
    if args.max_num_iterations is not None and args.max_num_iterations > 0:
        args.max_num_epochs = int(
            np.ceil(args.max_num_iterations / len(training_dataloader))
        )
    else:
    ...
    for epoch in range(
    ...
    ):  # This +1 is to ensure current_step can reach args.max_num_iterations
        for count, batch in enumerate(
            ...
            if (
                args.validate_every is not None
                and current_step % args.validate_every == 0
            ) or (args.validate_per_epoch and count == len(training_dataloader) - 1):
                model.eval()
                ...
                early_stop = (
                    esh.check(val_score_history[-1][1])
            ...
            if (
                args.save_every is not None and current_step % args.save_every == 0
            ) or (args.save_per_epoch and count == len(training_dataloader) - 1):
                model.eval()
                save_model(
                    model, tokenizer, current_step, args.model_folder, args.max_to_keep
                )
                model.train()

            if current_step >= args.max_num_iterations or early_stop:
                if not os.path.exists(
                    args.model_folder + "/checkpoint-%d" % (current_step)
                ):
                    model.eval()
                    save_model(
                        model,
    ...
    if len(val_score_history) > 0:
        best_checkpoint_batch_number = sorted(val_score_history, key=lambda x: -x[1])[
            0
        ][0]
    else:
    ...
    if best_checkpoint_batch_number is not None:
        print(
            "The best checkpoint on validation set is: %s,"
            % (args.model_folder + "/checkpoint-%d" % (best_checkpoint_batch_number))
        )
        ...
        convert_hf_to_ct2(
            model=hf_model_folder, output_dir=ct2_model_folder, quantization="float16"
        )
