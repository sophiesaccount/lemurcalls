...
def load_audio_and_label(
    ...
    for audio_path, label_path in tqdm(
        ...
        valid_indices = np.logical_and(
            np.logical_and(onset_arr < len(y) / 48000, offset_arr > 0),
        ...
        cluster_id_arr = np.array(
            [cluster_codebook[value] for value in label["cluster"]]
        )

        label.update(
            {"onset": onset_arr, "offset": offset_arr, "cluster_id": cluster_id_arr}
        )
...
def slice_audio_and_label(audio, label, total_spec_columns):
    ...
    for pos in range(0, len(padded_audio), num_samples_in_clip):
        ...
        intersected_indices = np.logical_and(
            padded_label["onset"] < end_time, padded_label["offset"] > start_time
        )
        label_clip = deepcopy(label)
        label_clip.update(
            {
                "onset": np.maximum(
                    padded_label["onset"][intersected_indices], start_time
                )
                - start_time,
                "offset": np.minimum(
                    padded_label["offset"][intersected_indices], end_time
                )
                - start_time,
                "cluster_id": padded_label["cluster_id"][intersected_indices],
                "cluster": [
                    padded_label["cluster"][idx]
...
def slice_audios_and_labels(audio_list, label_list, total_spec_columns):
    ...
    for audio, label in zip(audio_list, label_list):
        sliced_audios, sliced_labels = slice_audio_and_label(
            audio, label, total_spec_columns
        )
