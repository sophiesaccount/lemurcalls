...
def group_by_file(all_preds, all_labels, metadata_list): 
    ...
    for i in range(len(all_preds.get("onset", []))): 
        k = all_preds["orig_idx"][i] 
        preds_grouped[k]["onset"].append(all_preds["onset"][i]) 
        preds_grouped[k]["offset"].append(all_preds["offset"][i]) 
        preds_grouped[k]["cluster"].append(all_preds["cluster"][i]) 
        preds_grouped[k]["score"].append(all_preds["score"][i])

        labels_grouped = defaultdict(lambda: {"onset": [], "offset": [], "cluster": [], "quality": []})
        for i in range(len(all_labels.get("onset", []))):
            k = all_labels["orig_idx"][i]
            labels_grouped[k]["onset"].append(all_labels["onset"][i])
            labels_grouped[k]["offset"].append(all_labels["offset"][i])
            labels_grouped[k]["cluster"].append(all_labels["cluster"][i])
            labels_grouped[k]["quality"].append(all_labels["quality"][i])
        return preds_grouped, labels_grouped
"""
...
def filter_by_class(data_dict, target_class):
    ...
    return {
        ...
        "score": [
            s for s, m in zip(data_dict.get("score", [0] * len(mask)), mask) if m
        ],
    }
...
def filter_by_class_labels(data_dict, target_class):
    ...
    return {
        ...
        "cluster": [c for c, m in zip(data_dict["cluster"], mask) if m],
        "quality": [
            s for s, m in zip(data_dict.get("quality", [0] * len(mask)), mask) if m
        ],
    }
...
def evaluate_detection_metrics_with_false_class_qualities(
    labels, predictions, overlap_tolerance, allowed_qualities=None
):
    ...
    # beta = 2.0
    # eigentlcih f2 will aber nicht alles anpassen
...
def losses_val(
    ...
    if class_weights is not None:
        # class_weights -> [C] -> [1,1,C]
        weights = class_weights.view(1, 1, -1).to(out_cls_logits.device)

        # Focal loss for all classes (reduction='none')
        ...
        cls_loss = sigmoid_focal_loss(
            out_cls_logits, gt_cls_labels, reduction="sum"
        ) / max(num_pos, 1)
...
def load_actionformer_model(
    ...
    whisper_source = _resolve_whisper_source(
        whisper_size, whisper_load_source, whisper_model_id
    )
...
def actionformer_validation_f1_allclasses(
    model,
    dataloader,
    device,
    iou_threshold,
    ...
    final_preds = reconstruct_predictions(
        ...
        ID_TO_CLUSTER=ID_TO_CLUSTER,  # from datautils
    )
...
def evaluate(
    ...
    if "score" in final_preds:
        ...
        print("No scores in final_preds")
    ...
    # print(f' all_preds_final: {all_preds_final}')
    ...
    precision = (
        tp_total / (tp_total + fp_total + fc_total)
        if (tp_total + fp_total + fc_total) > 0
        else 0
    )
    recall = (
        tp_total / (tp_total + fn_total + fc_total)
        if (tp_total + fn_total + fc_total) > 0
        else 0
    )
    f1_all = (
...
def collate_fn(batch):
    ...
    return {
        ...
        "raw_labels": raw_labels,  # List of original labels (len = B) not needed anymore
    }
...
if __name__ == "__main__":
    ...
    if args.class_weights == True:
        class_weights, counts = compute_class_weights_from_label_list(
            label_list_train, cluster_codebook
        )
    else:
        class_weights = None

    # slices audios in chunks of total_spec_columns spectogram columns and adjusts the labels accordingly
    audio_list_train, label_list_train, metadata_list = slice_audios_and_labels(
    ...
    if args.val_ratio > 0:
        audio_list_val, label_list_val = load_data(
            audio_path_list_val,
            label_path_list_val,
            cluster_codebook=cluster_codebook,
            n_threads=1,
        ...
        audio_list_val, label_list_val, metadata_list_val = slice_audios_and_labels(
            audio_list_val, label_list_val, args.total_spec_columns
        )
    ...
    training_dataset = WhisperFormerDatasetQuality(
        audio_list_train,
        label_list_train,
        args.total_spec_columns,
        feature_extractor,
    ...
    for epoch in range(args.max_num_epochs + 1):
        ...
        for count, batch in enumerate(
            ...
            cls_loss, reg_loss, total_loss = actionformer_train_iteration(
                model, batch, optimizer, scheduler, scaler, device
            )
            ...
            if count % 100 == 0:
                print(
                    f"Epoch {epoch}, Step {count}, Training Total Loss: {total_loss:.4f}"
                )
                print(
                    f"Epoch {epoch}, Step {count}, Training Class Loss: {cls_loss:.4f}"
                )
                print(
                    f"Epoch {epoch}, Step {count}, Training Regression Loss: {reg_loss:.4f}"
                )
        ...
        print(
            f"Epoch {epoch}, Step {count}, Epoch Training Loss: {epoch_train_loss:.4f}"
        )
        print(
            f"val_ratio = {args.val_ratio}, will run validation: {args.val_ratio > 0}"
        )
        ...
        if args.val_ratio > 0 or args.label_folder_val:
            ...
            final_preds = actionformer_validation_f1_allclasses(
                model,
                val_dataloader,
                device,
                args.iou_threshold,
            ...
            if args.lr_schedule == "cosine":
                if epoch > 5:
                    ...
                    if new_lr < old_lr:
                        print(
                            f"LR reduced from {old_lr:.2e} to {new_lr:.2e} at epoch {epoch}"
                        )
    ...
    if len(f1_scores_val) > 0:
        ...
        for i in range(len(thresholds)):
            plt.plot(
                [sublist[i] for sublist in f1_scores_val],
                label=f"Validation F1 for threshold {thresholds[i]}",
            )
