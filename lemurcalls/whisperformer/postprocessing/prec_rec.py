...
from ..train import (
    collate_fn,
    group_by_file,
    evaluate_detection_metrics_with_false_class_qualities,
)
...
def evaluate_detection_metrics_with_false_class_qualities_q3_q2(
    labels, predictions, overlap_tolerance, allowed_qualities=[1, 2, 3]
):
    ...
    for p_idx, (po, pf, pc) in enumerate(zip(pred_onsets, pred_offsets, pred_clusters)):
        for l_idx, (lo, lf, lc, lq) in enumerate(
            zip(label_onsets, label_offsets, label_clusters, label_qualities)
        ):
    ...
    for p_idx, (po, pf, pc) in enumerate(zip(pred_onsets, pred_offsets, pred_clusters)):
        for l_idx, (lo, lf, lc, lq) in enumerate(
            ...
            if (
                l_idx in matched_labels
                or l_idx in q2_q3_matched_labels
                or p_idx in matched_preds
                or int(float(lq)) == 1
            ):
                continue
            inter = max(0.0, min(pf, lf) - max(po, lo))
    ...
    fn = (
        len(label_onsets) - len(matched_labels) - len(fn_q3_q2)
    )  # do not punish false negatives from qc 3 and 2
    fc = false_class
...
def group_by_file(all_preds, all_labels, metadata_list):
    ...
    preds_grouped = defaultdict(
        lambda: {"onset": [], "offset": [], "cluster": [], "score": []}
    )
    ...
    labels_grouped = defaultdict(
        lambda: {"onset": [], "offset": [], "cluster": [], "quality": []}
    )
...
if __name__ == "__main__":
    ...
    label_stems = [
        os.path.basename(label_path).split(".")[0] for label_path in label_path_list_val
    ]
    ...
    thresholds = (
        args.thresholds if args.thresholds else [0.1, 0.15, 0.2, 0.25, 0.3, 0.35]
    )
    ...
    for threshold in thresholds:
        ...
        json_files = sorted(
            [
                ...
                if f.endswith((".json", ".jsonr")) and f != "run_arguments.json"
            ]
        )
        ...
        for i, file_name in enumerate(json_files):
            ...
            with open(file_path, "r") as f:
                try:
                    ...
                    print(
                        f"[WARN] Datei {file_name} ist keine gültige JSON-Datei — wird übersprungen."
                    )
                    continue
            ...
            if not all(k in preds for k in required_keys):
                print(
                    f"[WARN] Datei {file_name} hat nicht das erwartete Format — wird übersprungen."
                )
                continue
            ...
            preds["cluster"] = [
                ID_TO_CLUSTER[FIXED_CLUSTER_CODEBOOK[c]] for c in clusters
            ]
            ...
            if pred_stem not in stem_to_idx:
                print(
                    f"[WARN] Kein Matching für Prediction {file_name} – wird übersprungen."
                )
                continue
            ...
            if "score" in preds:
                ...
                if not filtered:
                    continue
                preds["onset"], preds["offset"], preds["cluster"], preds["score"] = zip(
                    *filtered
                )
        ...
        all_preds_grouped, all_labels_grouped = group_by_file(
            all_preds, all_labels, metadata_list_val
        )
    ...
    plt.scatter(
        best_recall, best_precision, color="red", s=80, zorder=5, label="Best F1"
    )
    ...
    for i in range(0, len(thresholds), 5):
        ...
        # F1 beschriften
        plt.text(th, f1s[i] + 0.01, f"{th:.2f}", fontsize=7, ha="center", color="black")

        # F2 beschriften, etwas tiefer setzen
        plt.text(th, f2s[i] - 0.02, f"{th:.2f}", fontsize=7, ha="center", color="gray")

    # Beste F1 markieren
    best_idx_f1 = np.argmax(f1s)
    plt.scatter(thresholds[best_idx_f1], f1s[best_idx_f1], color="red", s=80, zorder=5)
    plt.text(
        thresholds[best_idx_f1],
        f1s[best_idx_f1] + 0.03,
    ...
    plt.text(
        thresholds[best_idx_f2],
        f2s[best_idx_f2] + 0.03,
