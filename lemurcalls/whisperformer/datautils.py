...
def get_audio_and_label_paths_from_folders(audio_folder, label_folder):
    ...
    """
    audio_files = {
        os.path.splitext(f)[0]: os.path.join(audio_folder, f)
...
def get_audio_and_label_paths_from_folders(audio_folder, label_folder):
    ...
    label_files = [
        f for f in os.listdir(label_folder) if f.endswith((".json", ".jsonr"))
    ]
...
FIXED_CLUSTER_CODEBOOK = {"m": 0, "t": 1, "w": 2, "lt": 1, "h": 1}
"""
...
def load_audio_and_label(
    ...
    for audio_path, label_path in tqdm(
        zip(audio_path_list, label_path_list),
        desc="load_data",
        ...
        valid_indices = np.logical_and(
            np.logical_and(onset_arr < len(y) / label["sr"], offset_arr > 0),
        ...
        label["cluster"] = [
            label["cluster"][idx] for idx in np.argwhere(valid_indices)[:, 0]
        ]
        if "quality" in label:
            label["quality"] = [
                label["quality"][idx] for idx in np.argwhere(valid_indices)[:, 0]
            ]
        else:
            label["quality"] = ["unknown"] * len(label["cluster"])
        cluster_id_arr = np.array(
            [cluster_codebook[str(value)] for value in label["cluster"]]
        )
...
def load_data(audio_path_list, label_path_list, cluster_codebook=None, n_threads=1):
    ...
    for thread_id, offset in enumerate(
        range(0, len(audio_path_list), samples_per_thread)
    ):
        t = threading.Thread(
...
def split_audio_and_label(audio, label, split_ratio):
    ...
    label_part1.update(
        {
            ...
            "offset": np.minimum(
                label["offset"][intersected_indices_part1], split_time
            ),
            "cluster_id": label["cluster_id"][intersected_indices_part1],
            "cluster": [
                label["cluster"][idx]
                for idx in np.argwhere(intersected_indices_part1)[:, 0]
            ],
        }
    )
    ...
    label_part2.update(
        {
            "onset": np.maximum(label["onset"][intersected_indices_part2], split_time)
            - split_time,
            "offset": label["offset"][intersected_indices_part2] - split_time,
            "cluster_id": label["cluster_id"][intersected_indices_part2],
            "cluster": [
                label["cluster"][idx]
                for idx in np.argwhere(intersected_indices_part2)[:, 0]
            ],
        }
    )
...
def train_val_split(audio_list, label_list, val_ratio):
    ...
    for audio, label in zip(audio_list, label_list):
        ...
        if mode == 0:
            (audio_val, label_val), (audio_train, label_train) = split_audio_and_label(
                audio, label, val_ratio
            )
        else:
            (audio_train, label_train), (audio_val, label_val) = split_audio_and_label(
                audio, label, 1 - val_ratio
            )
        ...
        for trial in range(num_trials):
            ...
            while start < len(audio):
                ...
                label_clip.update(
                    {
                        "onset": np.maximum(
                            label["onset"][intersected_indices], start_time
                        )
                        - start_time,
                        "offset": np.minimum(
                            label["offset"][intersected_indices], end_time
                        )
                        - start_time,
...
class VocalSegDataset(Dataset):
    ...
    def get_feature_extractor_bank(self, label_list):
        ...
        for label in label_list:
            key = "%s-%s-%s" % (
                str(label["sr"]),
                str(label["spec_time_step"]),
                str(label["min_frequency"]),
            )
            if key not in feature_extractor_bank:
                feature_extractor_bank[key] = WhisperFeatureExtractor(
    ...
    def map_time_to_spec_col_index(self, t, spec_time_step):
        ...
        return min(
            int(
                np.round(
                    t / (spec_time_step * RATIO_DECODING_TIME_STEP_TO_SPEC_TIME_STEP)
                )
            ),
    ...
    def __getitem__(self, idx):
        ...
        num_samples_in_clip = int(
            np.round(self.total_spec_columns * spec_time_step * sr)
        )

        clip_start = np.random.choice(
            min(num_samples_in_clip + 1, len(audio) - feature_extractor.n_fft + 1)
        )
        ...
        onset_in_clip = (
            np.maximum(label["onset"][intersected_indices], start_time) - start_time
        )
        offset_in_clip = (
            np.minimum(label["offset"][intersected_indices], end_time) - start_time
        )
        ...
        for pos in range(len(onset_in_clip)):
            label_text.append(
                ...
                % (
                    ...
                    self.map_time_to_spec_col_index(
                        offset_in_clip[pos], spec_time_step
                    ),
                )
            )
        ...
        decoder_input_ids = self.tokenizer.encode(
            label_text, max_length=self.max_length + 1, truncation=True, padding=True
        )
        labels = decoder_input_ids[1:]
        decoder_input_ids = decoder_input_ids[:-1]
        decoder_input_ids += [self.tokenizer.pad_token_id] * (
            self.max_length - len(decoder_input_ids)
        )
        ...
        output = {
            ...
            "decoder_input_ids": torch.tensor(
                np.array(decoder_input_ids), dtype=torch.int64
            ),
