...
class SpecViewer:
    def __init__(
        ...
        self.colors = [
            ...
            for color_string in list(mcolors.TABLEAU_COLORS.values())
        ...
        for color_arr in self.colors:
            if unique_colors is None:
                ...
                if np.all(unique_colors == color_arr, axis=1).sum() == 0:
                    unique_colors = np.concatenate(
                        [unique_colors, color_arr[np.newaxis, :]], axis=0
                    )
    ...
    def chunk_label(self, label, start_time, end_time):
        ...
        intersected_indices = np.logical_and(
            label_onset_arr < end_time, label_offset_arr > start_time
        )
        chunked_label = {
            "onset": (
                np.maximum(label_onset_arr[intersected_indices], start_time)
                - start_time
            ).tolist(),
            "offset": (
                np.minimum(label_offset_arr[intersected_indices], end_time) - start_time
            ).tolist(),
            "cluster": [
                label["cluster"][idx] for idx in np.argwhere(intersected_indices)[:, 0]
            ],
        }
    ...
    def plot_spec_and_labels(
        ...
        all_unique_clusters = sorted(
            list(set(list(label["cluster"]) + list(prediction["cluster"])))
        )
        cluster_color_mapper = {}
        for cluster in all_unique_clusters:
            if cluster not in cluster_color_mapper:
                cluster_color_mapper[cluster] = self.colors[
                    len(cluster_color_mapper) % len(self.colors)
                ]

        patches = [
            Patch(color=color, label=cluster)
            for cluster, color in cluster_color_mapper.items()
        ]
        ...
        spec = feature_extractor(audio_chunked, sampling_rate=sr, padding="do_not_pad")[
            "input_features"
        ][0]

        ## convert spec to colorful (3 channel)
        spec_colorful = np.flip(
            self.cmap(self.min_max_norm(spec, min_spec_value, max_spec_value))[
                :, :, :3
            ],
            axis=0,
        )
        ...
        spec_xticks_labels = [
            xticks_format % (v)
            for v in spec_xticks_values * spec_time_step + start_time
        ]
        ...
        for pos in range(len(label_chunked["onset"])):
            ...
            if (
                ...
                and offset_idx
                == int(np.round(label_chunked["onset"][pos + 1] / spec_time_step))
        ...
        for pos in range(len(prediction_chunked["onset"])):
            ...
            offset_idx = int(
                np.round(prediction_chunked["offset"][pos] / spec_time_step)
            )
            ...
            if (
                ...
                and offset_idx
                == int(np.round(prediction_chunked["onset"][pos + 1] / spec_time_step))
        ...
        canvas_image = np.ones(
            (spec_colorful.shape[0] + 10 + 40 + 10 + 40, spec_labels_image.shape[1], 3)
        )
        canvas_image[: spec_colorful.shape[0], :, :] = spec_colorful
        canvas_image[
            spec_colorful.shape[0] + 10 : spec_colorful.shape[0] + 50, :, :
        ] = spec_preds_image
        canvas_image[
            spec_colorful.shape[0] + 60 : spec_colorful.shape[0] + 100, :, :
        ] = spec_labels_image

        fig, ax = plt.subplots(
            nrows=1, ncols=1, figsize=(10, 4), tight_layout=True, sharex=False
        )
        ...
        ax.text(
            -137, -20, "Wav file name: %s" % (audio_file_name), fontfamily="monospace"
        )
        ...
        plt.legend(
            handles=patches, loc="upper center", bbox_to_anchor=(0.5, -0.5), ncol=4
        )
        plt.show()
    ...
    def visualize(
        ...
        feature_extractor = WhisperSegFeatureExtractor(
            sr, window_size / spec_width, min_frequency, max_frequency
        )

        whole_spec = feature_extractor(audio, sampling_rate=sr, padding="do_not_pad")[
            "input_features"
        ][0]
...
def slice_audio_and_label(audio, label, sr, start_time, end_time):
    ...
    target_indices = np.argwhere(
        np.logical_and(onsets < end_time, offsets > start_time)
    )[:, 0]

    sliced_onsets = [max(0, onsets[idx] - start_time) for idx in target_indices]
    sliced_offsets = [
        min(offsets[idx] - start_time, end_time - start_time) for idx in target_indices
    ]
