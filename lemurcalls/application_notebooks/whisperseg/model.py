...
def download_model(model_path, ignore_cache=False):
    # ✅ 1. FALL: lokaler Pfad → direkt zurückgeben
    if os.path.exists(model_path):
        print(f"[INFO] Using local model path: {model_path}")
        return model_path

    # ✅ 2. FALL: HuggingFace Hub repo_id
    local_model_path = os.path.join("models", model_path.replace("/", "_"))

    if not os.path.exists(local_model_path) or len(os.listdir(local_model_path)) == 0:
        os.makedirs(local_model_path, exist_ok=True)
        print(f"[INFO] Downloading HF model: {model_path}")
...
def load_model(initial_model_path, total_spec_columns, dropout=0.0):
    ...
    with torch.no_grad():
        ...
        model.model.encoder.embed_positions.num_embeddings = (
            model.config.max_source_positions
        )
    ...
    tokenizer = WhisperTokenizer.from_pretrained(
        "nccratliri/whisper-large", language="english"
    )
    tokenizer.add_tokens(
        ["<|%d|>" % (i) for i in range(total_spec_columns + 1)], special_tokens=True
    )
    tokenizer.add_tokens(
        [v for k, v in model.config.species_codebook.items()], special_tokens=True
    )
...
class SegmenterBase:
    ...
    def get_sliced_audios_features(
        self, audio, sr, min_frequency, spec_time_step, num_trials
    ):
        feature_extractor = WhisperSegFeatureExtractor(
            sr, spec_time_step, min_frequency=min_frequency
        )
        ...
        for trial_id in range(num_trials):
            padding_time = (
                np.round(clip_duration * trial_id / num_trials / spec_time_step)
                * spec_time_step
            )
            num_padding_samples = int(padding_time * sr)
            audio_padded = np.concatenate(
                [audio_left_pad[len(audio_left_pad) - num_padding_samples :], audio],
                axis=0,
            )
            ...
            for pos in range(0, max(len(audio_padded), 1), audio_clip_length):
                ...
                audio_clip_padded = np.concatenate(
                    [
                        audio_clip,
                        np.zeros(audio_clip_length - len(audio_clip), dtype=np.float32),
                    ],
                    axis=0,
                )

                input_features = feature_extractor(
                    audio_clip_padded, sampling_rate=sr, padding="do_not_pad"
                )["input_features"][0]
                    ...
                    [
                        ...
                        * np.ones(
                            (
                                ...
                                self.total_spec_columns - input_features.shape[1],
                            )
                        ),
                    ],
                    axis=1,
                ...
                sliced_audios_features.append(
                    (trial_id, offset_time, input_features, len(audio_clip) / sr)
                )
    ...
    def generate_segment_text(
        ...
        all_threads = []
        num_examples_per_thread = int(
            np.ceil(len(sliced_audios_features) / len(self.device_list))
        )
        for thread_id, pos in enumerate(
            range(0, len(sliced_audios_features), num_examples_per_thread)
        ):
            t = threading.Thread(
                ...
                args=(
                    ...
                    status_monitor
                    if thread_id == 0
                    else None,  ## pass the status_monitor only to the first thread
                ),
            )
            t.start()
    ...
    def extract_segments(self, text, spec_time_step):
        ...
        for onset_text, cluster_id_text, offset_text in match_res_list:
            onset = (
                int(onset_text)
                * spec_time_step
                * RATIO_DECODING_TIME_STEP_TO_SPEC_TIME_STEP
            )
            offset = (
                int(offset_text)
                * spec_time_step
    ...
    def parse_generation(
        ...
        for trial_id in on_offset_list_of_trial:
            ...
            for on_offsets_clip in on_offset_list_of_trial[trial_id]:
                if (
                    ...
                    and merged_on_offset_list_of_trial[trial_id][-1][1]
                    == on_offsets_clip[0][0]
                    and merged_on_offset_list_of_trial[trial_id][-1][2]
                    == on_offsets_clip[0][2]
                ):
                    ## previous offset == current onset and the cluster type is the same, then we merge them

                    merged_on_offset_list_of_trial[trial_id][-1][1] = on_offsets_clip[
                        0
                    ][1]
        ...
        for trial_id in merged_on_offset_list_of_trial:
            ...
            merged_on_offset_list_of_trial[trial_id] = sorted(
                merged_on_offset_list_of_trial[trial_id], key=lambda x: x[0]
            )
            merged_on_offset_list_of_trial[trial_id] = [
                item
                for item in merged_on_offset_list_of_trial[trial_id]
                if item[1] - item[0] >= min_segment_length
            ]
            ...
            trials_results.append(
                {
                    "onset": list(pred_onsets),
                    "offset": list(pred_offsets),
                    "cluster": list(pred_clusters),
                }
            )
        ...
        if num_trials == 1:
            ...
            if consolidation_method == "clustering":
                ...
                final_prediction = self.consolidate_trials_by_clustering(
                    trials_results, eps, min_samples
                )
            else:
                final_prediction = self.consolidate_trials_by_voting(
                    trials_results, time_per_frame_for_voting
                )

        ##formating the final prediction
        final_prediction["onset"] = [
            float(np.round(t, self.precision_bits)) for t in final_prediction["onset"]
        ]
        final_prediction["offset"] = [
            float(np.round(t, self.precision_bits)) for t in final_prediction["offset"]
        ]
    ...
    def consolidate_trials_by_clustering(self, trials, eps, min_samples):
        ...
        for trial_id, trial in enumerate(trials):
            for onset, offset, cluster in zip(
                trial["onset"], trial["offset"], trial["cluster"]
            ):
                segments.append(
                    {
                        "onset": onset,
        ...
        dist_matrix = pairwise_distances(
            [[seg["onset"], seg["offset"]] for seg in segments],
            metric=self.custom_distance,
        )
        ...
        for label in set(labels):
            if label != -1:  # Ignore noise
                group_segments = [
                    seg for seg, lbl in zip(segments, labels) if lbl == label
                ]
                ...
                for seg in group_segments:
                    cluster_name_dict[seg["cluster"]] = (
                        cluster_name_dict.get(seg["cluster"], 0) + 1
                    )
                ## get the most common cluster name
                cluster_name = sorted(
                    list(cluster_name_dict.items()), key=lambda x: -x[1]
                )[0][0]

                avg_onset = np.mean([seg["onset"] for seg in group_segments])
                avg_offset = np.mean([seg["offset"] for seg in group_segments])
                merged_segments.append(
                    {"onset": avg_onset, "offset": avg_offset, "cluster": cluster_name}
                )
    ...
    def consolidate_trials_by_voting(self, trials, time_per_frame_for_voting):
        ...
        for trial in trials:
            ...
            for pos in range(len(trial["onset"])):
                ...
                frame_wise_prediction[
                    int(np.round(onset / time_per_frame_for_voting)) : int(
                        np.round(offset / time_per_frame_for_voting)
                    )
                ] = cluster_id
        ...
        voted_frame_wise_prediction_right_pad = np.array(
            voted_frame_wise_prediction.tolist() + [-1]
        )
        voted_frame_wise_prediction_left_pad = np.array(
            [-1] + voted_frame_wise_prediction.tolist()
        )
        event_positions = np.argwhere(
            voted_frame_wise_prediction_right_pad - voted_frame_wise_prediction_left_pad
            != 0
        )[:, 0]
        ...
        for idx in range(0, len(event_positions) - 1):
            ...
            cluster_id = int(
                np.round(np.mean(voted_frame_wise_prediction[onset_pos:offset_pos]))
            )
    ...
    def segment(
        ...
        sliced_audios_features = self.get_sliced_audios_features(
            audio, sr, min_frequency, spec_time_step, num_trials
        )
        tic2 = time.time()
    ...
    def compute_syllable_score(
        self, prediction_on_offset_list, label_on_offset_list, tolerance=0.02
    ):
        ...
        for pred_onset, pred_offset, pred_cluster in prediction_on_offset_list:
            is_matched = False
            for count, (label_onset, label_offset, label_cluster) in enumerate(
    ...
    def segment_score(self, prediction, label, target_cluster=None, tolerance=0.02):
        ...
        for pos in range(len(prediction["onset"])):
            if target_cluster is None or str(target_cluster) == str(
                prediction["cluster"][pos]
            ):
                prediction_on_offset_list.append(
                    [
                        prediction["onset"][pos],
                        prediction["offset"][pos],
                        str(prediction["cluster"][pos]),
                    ]
                )
        ...
        for pos in range(len(label["onset"])):
            if target_cluster is None or str(target_cluster) == str(
                ...
                label_on_offset_list.append(
                    [
                        label["onset"][pos],
                        label["offset"][pos],
                        str(label["cluster"][pos]),
                    ]
                )

        if target_cluster is not None and len(label_on_offset_list) == 0:
            print(
                "Warning: the specified target cluster '%s' does not exist in the ground-truth labels."
                % (str(target_cluster))
            )

        TP, P_pred, P_label = self.compute_syllable_score(
            prediction_on_offset_list, label_on_offset_list, tolerance
        )
    ...
    def frame_score(
        self, prediction, label, target_cluster=None, time_per_frame_for_scoring=0.01
    ):
        ...
        for cluster in list(prediction_segments["cluster"]) + list(
            label_segments["cluster"]
        ):
        ...
        for idx in range(len(prediction_segments["onset"])):
            onset_pos = int(
                np.round(prediction_segments["onset"][idx] / time_per_frame_for_scoring)
            )
            offset_pos = int(
                np.round(
                    prediction_segments["offset"][idx] / time_per_frame_for_scoring
                )
            )
            frame_wise_prediction[onset_pos:offset_pos] = cluster_to_id_mapper[
                prediction_segments["cluster"][idx]
            ]

        frame_wise_label = np.ones(num_frames) * -1
        for idx in range(len(label_segments["onset"])):
            onset_pos = int(
                np.round(label_segments["onset"][idx] / time_per_frame_for_scoring)
            )
            offset_pos = int(
                np.round(label_segments["offset"][idx] / time_per_frame_for_scoring)
            )
            frame_wise_label[onset_pos:offset_pos] = cluster_to_id_mapper[
                label_segments["cluster"][idx]
            ]

        if target_cluster is None:
            TP = np.logical_and(
                frame_wise_label != -1, frame_wise_prediction == frame_wise_label
            ).sum()
            ...
            TP = np.logical_and(
                frame_wise_label == target_cluster_id,
                frame_wise_prediction == frame_wise_label,
            ).sum()
...
class WhisperSegmenterForEval(SegmenterBase):
    def __init__(self, model_path=None, device=None, model=None, tokenizer=None):
        ...
        self.inverse_cluster_codebook = {
            cluster_id: cluster for cluster, cluster_id in self.cluster_codebook.items()
        }
    ...
    def generate_segment_text(
        ...
        for pos in range(0, len(sliced_audios_features), batch_size):
            input_features = torch.tensor(
                [item[2] for item in sliced_audios_features[pos : pos + batch_size]],
                dtype=torch.float32,
            ).to(self.device)
...
class WhisperSegmenter(SegmenterBase):
    def __init__(
        ...
        if device == "cpu":
            ...
            self.model_list = [
                WhisperForConditionalGeneration.from_pretrained(model_path).to(device)
                for device in self.device_list
            ]
        self.tokenizer_list = [
            WhisperTokenizer.from_pretrained(model_path, language="english")
        ...
        for pos in range(0, len(sliced_audios_features), batch_size):
            ...
            if status_monitor is not None:
                progress_percent = int(
                    100 * min(1, (pos + batch_size) / len(sliced_audios_features))
                )
...
class WhisperSegmenterFast(SegmenterBase):
    def __init__(
        ...
        self.tokenizer_list = [
            WhisperTokenizer.from_pretrained(
                model_path + "/hf_model", language="english"
            )
            for _ in self.device_list
        ]
    ...
    def generate_segment_text_core(
        ...
        for pos in range(0, len(sliced_audios_features), batch_size):
            ...
            """
            sliced_audios_features_batch = sliced_audios_features[
                pos : pos + batch_size
            ]
            actual_batch_size = len(sliced_audios_features_batch)
            features = ctranslate2.StorageView.from_array(
                np.asarray([item[2] for item in sliced_audios_features_batch])
            )
            ...
            model_output = model.generate(
                features,
                [prompt for _ in range(actual_batch_size)],
                max_length=max_length,
                beam_size=num_beams,
                sampling_topk=top_k,
                length_penalty=length_penalty,
