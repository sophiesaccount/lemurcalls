...
def confusion_matrix_framewise(
    ...
    label: np.ndarray,
    time_per_frame_for_scoring=0.01,
    name: str = None,
):
    ...
    for idx in range(len(prediction_segments["onset"])):
        ...
        offset_pos = int(
            np.round(prediction_segments["offset"][idx] / time_per_frame_for_scoring)
        )
    ...
    print(
        classification_report(
            ...
            labels=labels_num,
            target_names=labels_alpha,
        )
    )
    if name != None:
        plt.savefig(
            f"/usr/users/bhenne/projects/whisperseg/results/{name}_cm_framewise.png",
            format="png",
            dpi=400,
            bbox_inches="tight",
        )
        ...
        plt.savefig(
            f"/usr/users/bhenne/projects/whisperseg/results/{current_time}_cm_framewise.png",
            format="png",
            dpi=400,
            bbox_inches="tight",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a confusion matrix from frame-wise scores loaded from a file"
    )
    parser.add_argument(
        "--prediction_file",
        type=str,
    ...
    parser.add_argument(
        "--label_file", type=str, help="Path to file containing labels", required=True
    )
