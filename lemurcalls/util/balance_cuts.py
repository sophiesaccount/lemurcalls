...
def clip_fragment(
    ...
    end_frame = int(
        min(offset * sampling_rate + (post_pad * sampling_rate), max_frames)
    )

    trimmed_data = data[start_frame:end_frame]
    new_file_name = join(
        output_dir, Path(wav_file).stem + f"_seg_{num_fragments:03}.wav"
    )
...
def clip_json(
    ...
    new_file_name = join(
        output_dir, Path(json_file).stem + f"_seg_{num_fragments:03}.json"
    )
    ...
    for fpath in get_flex_file_iterator(file_path, rglob_str="*.json"):
        ...
        while i < len(json_data["onset"]) - 1:
            if abs(json_data["offset"][i] - json_data["onset"][i + 1]) > max_silence:
                ...
                pre_pad = clip_fragment(
                    ...
                    pre_distance=(
                        abs(json_data["offset"][start - 1] - json_data["onset"][start])
                        if start > 1
                        else 1
                    ),  # else 1 = safety padding from trim_wavs.py
                    # pre_distance=(abs(json_data['offset'][i-1] - json_data['onset'][i]) if i > 1 else 1),
                    post_distance=abs(
                        json_data["offset"][i] - json_data["onset"][i + 1]
                    ),
        ...
        if start < len(json_data["onset"]):
            pre_pad = clip_fragment(
                ...
                post_distance=float(
                    "inf"
                ),  # any length, clip_fragment checks for end-of-file
                output_dir=output_dir,
            )
            clip_json(
                json_file=fpath,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Finds gaps larger than '-s' seconds between calls in a audio file and splits it into fragments that contain a more balanced amount of calls to background noise."
    )
    parser.add_argument(
        "-p",
        "--file_path",
    ...
    parser.add_argument(
        ...
        "--max_silence",
        type=int,
        help="Longest permitted interval between two calls, in seconds.",
        required=True,
    )
