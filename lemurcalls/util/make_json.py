...
import yaml
from common import (
    compute_annotation_metadata,
    compute_spec_time_step,
...
def make_json(
    ...
    if not filter and convert_annotations in [
        "parent_filter_replace",
        "animal_filter_replace",
        "parent_filter_drop",
        "animal_filter_drop",
    ]:
        print(
            f'make_json.py: argument -a/--convert_annotations: "{convert_annotations}" requires a valid list of filters.'
        )
        exit(1)
    if convert_annotations == "none":
        ...
        if filter:
            transition_matrix = {
                vv: vv for v in classes.values() for vv in v if vv in filter
            }
        else:
        ...
        transition_matrix = {
            vv: (k.lower() if vv not in filter else vv)
        ...
        transition_matrix = {
            vv: k.lower() for k, v in classes.items() for vv in v if vv in filter
        }
        ...
        transition_matrix = {
            vv: ("vocal" if vv not in filter else vv)
        ...
        transition_matrix = {
            vv: "vocal" for v in classes.values() for vv in v if vv in filter
        }
    for p in get_flex_file_iterator(file_path, rglob_str="*.txt"):
        ...
        hop_length = compute_annotation_metadata(
            duration=clip_duration, sampling_rate=sampling_rate
        )
        ...
        content = {
            ...
            "min_segment_length": float(
                min([l[7] for l in lines])
            ),  # Delta Time (s) column in Raven tables
            "tolerance": tolerance,
        ...
        if len(invalid) > 0:
            logging.warning(
                f"Invalid labels found in {p}: {sorted(set(invalid))}. These have been ignored in the output file."
            )
        if output_path == "":
    ...
    smallest_segment = min(
        [content["min_segment_length"] for content in completed_files.values()]
    )
    for file_name, content in completed_files.items():
        ...
        if merge_targets:
            content["cluster"] = [
                "target" if label != "vocal" else "vocal"
                for label in content["cluster"]
            ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Prepares Raven selection tables for WhisperSeg use: convert to .json and add some more info"
    )
    parser.add_argument(
        "-p",
        "--file_path",
        type=str,
        help="Path to .txt selection table files",
        required=True,
    )
    parser.add_argument(
        "-c",
    ...
    parser.add_argument(
        ...
        const="none",
        default="none",
        help="Whether to abstract annotations into their parent classes or leave them unchanged. 'none' uses all annotations as provided, 'parent' converts all child annotations into their parent class, 'animal' converts all annotations to \"vocal\". 'x_filter_replace' only converts labels that do not match any filter. 'x_filter_drop' only converts labels that match a filter and drops the rest. Defaults to %(default)s.",
    )
    parser.add_argument(
        "-f",
        "--filter",
        ...
        default=None,
        metavar="string",
        help="List of allowed labels to use",
    )
    parser.add_argument(
        "--merge_targets",
        action="store_true",
        help='Merge all non-"vocal" annotations into a single "target" group.',
    )
    parser.add_argument(
        "-s",
        "--species",
        type=str,
        help='The species in the audio, e.g., "zebra_finch" When adding new species, go to the WhisperSeg load_model() function in model.py, add a new pair of species_name:species_token to the species_codebook variable. E.g., "catta_lemur":"<|catta_lemur|>".',
        default="catta_lemur",
    )
    parser.add_argument(
        "-d",
        "--clip_duration",
        type=float,
        help="Length D of the audio clips each file will be divided into by the algorithm. Used here to compute hop length L and spec_time_step.",
        default=2.5,
    )
    parser.add_argument(
        "-m",
        "--min_frequency",
        type=int,
        help="The minimum frequency when computing the Log Melspectrogram. Frequency components below min_frequency will not be included in the input spectrogram.",
        default=0,
    )
    parser.add_argument(
        "-t",
        "--tolerance",
        type=float,
        help="When computing the F1_seg score, we need to check if both the absolute difference between the predicted onset and the ground-truth onset and the absolute difference between the predicted and ground-truth offsets are below a tolerance (in seconds)",
        default=0.5,
    )
    parser.add_argument(
        "-i",
        "--time_per_frame",
        type=float,
        help="The time bin size (in second) used when computing the F1_frame score.",
        default=0.001,
    )
    parser.add_argument(
        "-e",
        "--epsilon",
        type=float,
        help="The threshold epsilon_vote during the multi-trial majority voting when processing long audio files",
        default=0.02,
    )
    parser.add_argument(
        "-o",
        "--output_path",
        type=str,
        help="Path to the output .json file. Defaults to the input directory.",
        default="",
    )
