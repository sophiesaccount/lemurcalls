...
def confusion_matrix_framewise(
    prediction: np.ndarray, label: np.ndarray, cluster_to_id_mapper: dict
):
    ...
    labels_alpha, labels_num = zip(
        *sorted(zip(cluster_to_id_mapper.keys(), cluster_to_id_mapper.values()))
    )
    ...
    plt.savefig(
        f"D:\\work\\whisperseg\\results\\{current_time}_cm_framewise.pdf",
        format="pdf",
        dpi=400,
        bbox_inches="tight",
    )
...
def gather_predictions(
    data_path: str, time_per_frame_for_scoring=0.001, **kwargs
    ...
    for file in get_flex_file_iterator(data_path, rglob_str="*.cmraw"):
        ...
        if not (superset := set(cluster_to_id_mapper.keys())).issuperset(
            subset := set(
                list(prediction_segments["cluster"]) + list(label_segments["cluster"])
            )
        ):
            raise ValueError(
                f'Unexpected annotations "{superset.difference(subset)}" found.'
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Builds a single confusion matrix from the results of a whole experiment across all data configs."
    )
    parser.add_argument(
        "-d", "--data_path", type=str, help="Path to the data", required=True
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        type=str,
        help="Path to a directory where the output file will be saved",
        default=None,
    )
    ...
    confusion_matrix_framewise(
        ...
        cluster_to_id_mapper=id_mapper,
    )  # doesnt pass output_dir currently
