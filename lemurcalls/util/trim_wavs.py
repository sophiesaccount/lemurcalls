...
def trim_wavs(
    file_path: str, safety_padding: float = 1.0, skip_wavs: bool = False
) -> None:
    ...
    for p in get_flex_file_iterator(file_path, rglob_str="*.wav"):
        ...
        if not skip_wavs:
            ...
            start_frame = int(
                max(onset * sampling_rate - (safety_padding * sampling_rate), 0)
            )
            end_frame = int(
                min(
                    offset * sampling_rate + (safety_padding * sampling_rate),
                    max_frames,
                )
            )
        ...
        if onset <= safety_padding:
            ...
            json_data["onset"] = [
                x - onset + safety_padding for x in json_data["onset"]
            ]
            json_data["offset"] = [
                x - onset + safety_padding for x in json_data["offset"]
            ]
    ...
    if not skip_wavs:
        csv_file_path = join(
            "results", datetime.now().strftime("%Y%m%d-%H%M%S") + "_meta_info.csv"
        )
        with open(csv_file_path, "w", newline="") as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow(
                ["Parent Path", "File Name", "Time Cut Front", "Time Cut Back"]
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Trim unannotated content from front and back of .wav audio recordings in given directory based on their onset and offset times from the associated json files. This overwrites the original .wav files with the trimmed versions and saves information about length of trimmed content to disk."
    )
    parser.add_argument(
        "-p",
        "--file_path",
        type=str,
        help="Path to directory containing .wav and .json files",
        required=True,
    )
    parser.add_argument(
        "-S",
        "--skip_wavs",
        action="store_true",
        help="Skip trimming the .wav files and only update the .json files",
        default=False,
    )
