...
def evaluate_detection_metrics_with_false_class_qualities(labels, predictions, overlap_tolerance, allowed_qualities = None):
    ...
    #print(f'label_onsets 1: {label_onsets}')
    #print(f'allowed_qualities: {allowed_qualities}' )
    ...
    if allowed_qualities is not None:
        ...
        """
    label_onsets   = np.array(label_onsets)
    label_offsets  = np.array(label_offsets)
    label_clusters = np.array(label_clusters)

    print(f'label_onsets 2: {label_onsets}')
    # load predictions
...
if __name__ == "__main__":
    ...
    if args.label_folder_val:
        audio_path_list_val, label_path_list_val = get_audio_and_label_paths_from_folders(
            args.audio_folder_val, args.label_folder_val)
