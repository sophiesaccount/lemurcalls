...
def evaluate_detection_metrics_with_false_class_qualities_q3(labels, predictions, overlap_tolerance, allowed_qualities = [1,2,3]):
    ...
    for p_idx, (po, pf, pc) in enumerate(zip(pred_onsets, pred_offsets, pred_clusters)):
        for l_idx, (lo, lf, lc, lq) in enumerate(zip(label_onsets, label_offsets, label_clusters, label_qualities)):
            if l_idx in matched_labels or p_idx in matched_preds or int(float(lq)) == 3:
                continue
    ...
    for p_idx, (po, pf, pc) in enumerate(zip(pred_onsets, pred_offsets, pred_clusters)):
        for l_idx, (lo, lf, lc, lq) in enumerate(zip(label_onsets, label_offsets, label_clusters, label_qualities)):
            if l_idx in matched_labels  or l_idx in q2_q3_matched_labels or p_idx in matched_preds or int(float(lq)) != 3:
                continue
            inter = max(0.0, min(pf, lf) - max(po, lo))
            union = max(pf, lf) - min(po, lo)
            ov = inter / union if union > 0 else 0.0
...
if __name__ == "__main__":
 ...
 #---- get predictions for calculation of F1 val score ----#
    ...
    for file_idx in all_preds_grouped:
        ...
        metrics = evaluate_detection_metrics_with_false_class_qualities_q3(all_labels_grouped[file_idx], all_preds_grouped[file_idx], overlap_tolerance = args.overlap_tolerance, allowed_qualities = args.allowed_qualities)
